...
void os_process_arguments(int argc, char *argv[])
{
	...
	if (!f_setup.restore_mode) {
		...
		f_setup.save_name =
		    malloc((strlen(f_setup.auto_save_name) +
			    strlen(EXT_SAVE)) * sizeof(char) + 1);
		memcpy(f_setup.save_name, f_setup.auto_save_name,
		       (strlen(f_setup.auto_save_name) +
			strlen(EXT_SAVE)) * sizeof(char));
	...
	f_setup.mem_name = malloc((strlen(f_setup.story_name) +
	strlen(EXT_MEM)) * sizeof(char) + 1);
	memcpy(f_setup.mem_name, f_setup.story_name,
		(strlen(f_setup.story_name) + strlen(EXT_MEM)) * sizeof(char));
