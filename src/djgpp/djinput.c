/*
 * djinput.c - DJGPP interface, input functions
 *
...
static void switch_cursor(bool cursor)
{
	...
	if (display <= _TEXT_) {
		...
		if (display == _MONO_)
			regs.x.cx = overwrite ? 0x080f : 0x0a0b;
		else
			regs.x.cx = overwrite ? 0x0408 : 0x0506;
...
static int get_key(bool cursor)
{
	...
	do {
		...
		if (_bios_keybrd(_NKEYBRD_READY)) {
			word code = _bios_keybrd(_NKEYBRD_READ);
			byte code0 = code;
			byte code1 = code >> 8;
