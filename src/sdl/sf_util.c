...
void os_process_arguments(int argc, char *argv[])
{
	...
	strncpy(f_setup.script_name, f_setup.story_name, strlen(f_setup.script_name) + strlen(EXT_SCRIPT));
	...
	strncpy(f_setup.command_name, f_setup.story_name, strlen(f_setup.command_name) + strlen(EXT_COMMAND));
	...
	if (!f_setup.restore_mode) {
		...
		strncpy(f_setup.save_name, f_setup.story_name, strlen(f_setup.save_name) + strlen(EXT_SAVE));
		...
		f_setup.save_name =
		    malloc((strlen(f_setup.tmp_save_name) +
			    strlen(EXT_SAVE)) * sizeof(char) + 1);
		strncpy(f_setup.save_name, f_setup.tmp_save_name, strlen(f_setup.save_name) + strlen(EXT_SAVE));
	...
	strncpy(f_setup.aux_name, f_setup.story_name, strlen(f_setup.aux_name) + strlen(EXT_AUX));
