...
static void writeRIFFheader(unsigned char *f, int freq, int numsamples,
			    int numchannels, int bytespersample);

// returns 1 if resampling, 0 if not needed, negative if error
static int initCONV(CONV * conv, double ratio, int ncha, int bytespersam,
		    int maxinp);
...
int sf_aiffwav(FILE * f, int foffs, void **wav, int *length)
{
	...
	if (fread(chk, 1, 4, f) < 4)
		return -1;	// FORM
	if (memcmp(chk, "FORM", 4))
		return -3;
	if (fread(chk, 1, 4, f) < 4)
		return -1;	// size
	size = glong(chk);
	...
	if (fread(chk, 1, 4, f) < 4)
		return -1;	// AIFF
	if (memcmp(chk, "AIFF", 4))
		return -3;
	size -= 4;
	while (size > 8) {
		if (fread(fid, 1, 4, f) < 4)
			return -1;	// chunck id
		if (fread(chk, 1, 4, f) < 4)
			return -1;	// and len
		size -= 8;
...
//              if (!data) return -55;
	...
	*wav = (void *)bout;
	...
	if ((needCONV =
	     initCONV(&conv, cratio, channels, (bps + 7) / 8, BLOCKSIZE)) < 0) {
		free(bout);
		*wav = NULL;
		return -997;
	}
