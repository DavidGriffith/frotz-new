 ...
 */
	...
	numchunks = 0;
	chunks =
	    (bb_chunkdesc_t *) malloc(sizeof(bb_chunkdesc_t) * chunks_size);
			  ...
			  uint32 usage, int resnum)
{
	...
	found =
	    bsearch(&ptr, map->ressorted, map->numresources,
...
bb_err_t bb_get_palette(bb_map_t * map, bb_palette_t ** res)
{
	...
	if (!map->palette) {
		...
		if (chunkres.length == 1) {
			...
			bb_color_t *colors =
			    (bb_color_t *) malloc(size * sizeof(bb_color_t));
			if (!colors)
				return bb_err_Alloc;
			if (size < 1 || size > 256)
				return bb_err_Format;
